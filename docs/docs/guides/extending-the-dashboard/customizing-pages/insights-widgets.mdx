...
edited). Both drive a `refreshToken` counter exposed on `useWidgetFilters()`. To opt your widget
into these page-level refreshes, include the token in your React Query `queryKey` — a change to the
token then refetches your data, just like a change to the date range does:
...
export function CustomWidget() {
    ...
    const { data } = useQuery({
        // Fold `refreshToken` into the key so the page-level refresh button and the polling
        // interval both refetch this widget.
...
window focus, so a widget that omits the token simply won't participate in page-level refreshes.
